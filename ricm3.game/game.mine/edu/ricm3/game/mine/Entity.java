...
public class Entity {
	...
	boolean collisionTerrain() {
		...
		while (iter.hasNext()) {
			Rect r = iter.next();
			if (x + m_dx < r.m_x + r.m_size_x && x + m_dx + w > r.m_x && y + m_dy < r.m_y + r.m_size_y
					&& y + m_dy + h > r.m_y) {
				this.m_dx = 0;
				this.m_dy = 0;
				col = true;
	...
	boolean collisionGhost() {
		int x = m_hitbox_x;
		int y = m_hitbox_y;
		int w = m_hitbox_w;
		int h = m_hitbox_h;
		for(int i=1;i<=m_model.m_nbghosts;i++) {
			Ghost g = m_model.getGhost(i);
			if(x+m_dx < g.m_x+g.m_w && x+m_dx+w > g.m_x && y+m_dy < g.m_y+g.m_h && y+m_dy+h > g.m_y) {
				return true;
			}
		}	
		return false;
